...
@router.post("/{mcp_id}", response_model=Dict[str, Any])
async def select_mcp(mcp_id: str, current_user: dict = Depends(get_current_user)):
    """사용자가 MCP를 선택합니다."""
    result = await nosql_crud.select_mcp_for_user(str(current_user["_id"]), mcp_id)
    return result
...
async def deselect_mcp(mcp_id: str, current_user: dict = Depends(get_current_user)):
    """사용자의 MCP 선택을 취소합니다."""
    success = await nosql_crud.deselect_mcp(str(current_user["_id"]), mcp_id)
    if not success:
