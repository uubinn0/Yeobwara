...
async def get_env_variable(mcp_id: str, current_user: dict = Depends(get_current_user)):
    """특정 MCP에 대한 사용자의 환경 변수를 조회합니다."""
    env_var = await nosql_crud.get_env_vars(str(current_user["_id"]), mcp_id)
    if not env_var:
        raise HTTPException(status_code=404, detail="환경 변수를 찾을 수 없습니다.")
    return {"mcp_id": mcp_id, "env_settings": env_var}
...
async def delete_env_variable(mcp_id: str, current_user: dict = Depends(get_current_user)):
    """사용자의 MCP 환경 변수를 삭제합니다."""
    success = await nosql_crud.delete_env_var(str(current_user["_id"]), mcp_id)
    if not success:
        raise HTTPException(status_code=404, detail="환경 변수를 찾을 수 없습니다.")
    return {"success": True}
